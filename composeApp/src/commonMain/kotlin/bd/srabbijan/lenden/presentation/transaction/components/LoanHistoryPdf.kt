/*
package bd.srabbijan.smartpos.feature.customer.presentation.loanHistory.components
...
import bd.srabbijan.smartpos.feature.customer.domain.LoanHistory
import java.io.ByteArrayOutputStream


fun generateLoanHistoryPdf(
    context: Context,
    customer: Customer?,
    data: List<LoanHistory>,
...
/**//*
    ...
    customer?.let { cus ->
        drawCentered(cus.name)
        cus.number?.let {
            drawCentered(it)
        }
        if (cus.amount.isPositive()) {
            drawCentered("বাকি পাবো : ${cus.amount.toCurrencyFormat()}")
        } else {
            drawCentered("বাকি দিবো : ${cus.amount.toPositive().toCurrencyFormat()}")
        }
        drawLine()
    }
    ...
    // Table headers
    paint.textAlign = Paint.Align.LEFT
    canvas.drawText("তারিখ", margin.toFloat(), y.toFloat(), paint)

    paint.textAlign = Paint.Align.CENTER
    canvas.drawText(
        "দিয়েছি ${totalGivenAmount.toCurrencyFormat()}",
        (pageWidth / 2f),
        y.toFloat(),
        paint
    )
    ...
    paint.textAlign = Paint.Align.RIGHT
    canvas.drawText(
        "পরিশোধ ${totalTakenAmount.toCurrencyFormat()}",
        (pageWidth - margin).toFloat(),
        y.toFloat(),
        paint
    )
    ...
    data.onEach {
        checkPageSpace()
        paint.textAlign = Paint.Align.LEFT
        val nameHeight = drawMultilineText(
            it.updatedAt.convertApiDateToUIDate(),
            margin.toFloat(),
            y.toFloat(),
            200
        )
        ...
        paint.textAlign = Paint.Align.CENTER
        canvas.drawText(
            if (it.type == ActivityLogType.LOAN_GIVE.name) it.amount.toCurrencyFormat() else "---",
            (pageWidth / 2f), centerY, paint
        )

        paint.textAlign = Paint.Align.RIGHT
        canvas.drawText(
            if (it.type == ActivityLogType.LOAN_TAKE.name) it.amount.toPositive()
                .toCurrencyFormat() else "---",
            (pageWidth - margin).toFloat(), centerY, paint
        )
